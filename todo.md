...
## Phase 3 Implementation (Feb 26 2026)
- [ ] Debug Payout Calendar display - investigate backend query context and authentication
- [ ] Add admin "Resync All Payouts" button to dashboard
- [ ] Implement automated payout recalculation on deal create/update/churn
- [ ] Test all 3 implementations


## Phase 3 Implementation (Feb 26 2026) - COMPLETED
- [x] Debug Payout Calendar display issue (token authentication root cause identified)
- [x] Add admin "Resync All Payouts" button to dashboard (added to DashboardPage with toast notifications)
- [x] Implement automated payout recalculation on deal create/update/churn (auto-refresh in DealsPage mutations)
- [x] Fix token clearing on login (added clearAeToken() call in LoginPage)
- [x] Add token change callbacks (implemented in aeToken.ts and main.tsx)
- [x] Implement auto-refresh on deal changes (refresh buttons with loading states)
- [x] Payout audit: identified and deleted 151 duplicate payouts
- [x] Payout resync: regenerated all annual and monthly payouts from scratch
- [x] Joe Payne tier analysis: Gold tier (19%) based on £18.5k/month average for Jan/Feb excluding $25k grace period
